...
public class IntegerRange implements Iterable<Integer> {
	...
	/**
	 * Constructs a new IntegerRange object. The step will be set to -1 if
	 * end<start or to 1 otherwise.
	 * 
	 * @param start
	...
	public IntegerRange(int start, int end, int step) {
		if ((end - start) * step < 0)
			throw new IllegalArgumentException(
					"The step of an IntegerRange must have the correct sign.");
		if (step == 0)
			throw new IllegalArgumentException(
					"The step of an IntegerRange must not be 0");
		this.start = start;
		this.end = end;
		this.step = step;
	}
	...
	/**
	 * Returns the number of elements in this IntegerRange. That is <em>not</em>
	 * the length of the interval, but <code>(end-start)/step + 1</code>.
	 * 
	...
	/**
	 * Checks whether this contains the given number, i.e. whether the iterator
	 * will yield the number. This is different from interval containment:
	 * <code>0..2.by(2)</code> will <em>not</em> contain 1.
	 * 
	 * @param number
	...
	private class RangeIterator implements ListIterator<Integer> {
		...
		public void remove() {
			throw new UnsupportedOperationException(
					"Cannot remove elements from a Range");
		}

		public void set(Integer e) {
			throw new UnsupportedOperationException(
					"Cannot set elements in a Range");
		}
